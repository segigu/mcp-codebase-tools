...
import { securityAudit, type SecurityAuditOutput } from '../analyze/securityAudit.js'
import { a11yAudit, type A11yAuditOutput } from '../analyze/a11yAudit.js'
import { techDebtCalculator, type TechDebtOutput } from '../analyze/techDebtCalculator.js'
...
export async function fullProjectAudit(
  ...
  if (securityScore < 80) {
    recommendations.push('🔒 Security: Address critical and high-priority security vulnerabilities immediately')
  }

  if (a11yScore < 80) {
    recommendations.push('♿ Accessibility: Improve accessibility to meet WCAG standards')
  }

  if (techDebtScore < 70) {
    recommendations.push('🧹 Tech Debt: Allocate time to reduce technical debt before adding new features')
  }

  if (summary.criticalIssues > 0) {
    recommendations.push(`⚠️ Critical: ${summary.criticalIssues} critical issues require immediate attention`)
  }

  if (overallScore >= 90) {
    recommendations.push('✨ Excellent: Project health is excellent. Continue maintaining high standards.')
  } else if (overallScore >= 80) {
    recommendations.push('👍 Good: Project health is good. Focus on addressing remaining issues.')
  } else if (overallScore >= 70) {
    recommendations.push('⚠️ Fair: Project health needs improvement. Prioritize high-severity issues.')
  } else {
    recommendations.push('🚨 Poor: Project health is poor. Immediate action required on multiple fronts.')
  }
  ...
  logAuditResult(
    ...
    'mcp:full-project-audit',
    {
      ...
      stats: {
        ...
        security: healthScore.security,
        accessibility: healthScore.accessibility,
        techDebt: healthScore.techDebt
      },
      issues: prioritizedIssues.slice(0, 20)
    },
    executionTime
  )
